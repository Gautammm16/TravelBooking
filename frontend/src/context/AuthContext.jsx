...
//   const [otpEmail, setOtpEmail] = useState(null); // Track email for OTP flow
...
//     // Handle unverified users (except admin)
...
//       // For OTP verification flow, don't login immediately
//       if (data.data.user?.isVerified === false) {
//         setOtpEmail(userData.email); // Store email for OTP verification
//         return { 
//           success: true, 
...
//       // For immediate login (social login or no verification required)
//       localStorage.setItem('token', data.token);
//       api.defaults.headers.common['Authorization'] = `Bearer ${data.token}`;
...
//     const { data } = await api.post(`${API_BASE}/verifyEmailOTP`, 
//       { email, otp },
//       {
//         headers: {
//           'Content-Type': 'application/json',
//           // Remove Authorization header since this is public endpoint
//         }
//       }
//     );
...
//     // Special handling for expired OTPs
...
//       email: data.email // Forward the email from backend
//     };
//   } catch (err) {

...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if registration was successful and requires OTP verification
      if (response && response.success) {
        // Navigate to OTP verification page with email as state
        navigate('/verify-otp', { state: { email } });
      } else {
        // Handle other cases (like social login that doesn't need verification)
        navigate('/login');
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 mb-2" htmlFor="lastname">Lastname</label>
            <input
              type="text"
              id="lastName"
              value={lastName}

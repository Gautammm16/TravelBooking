...
export const createBooking = catchAsync(async (req, res, next) => {
  const { tour, price, paymentMethod, participants, startDate, paymentData } = req.body;
  ...
  // If payment method is Razorpay, verify payment first
  if (paymentMethod === 'razorpay') {
    if (!paymentData) {
      return next(new AppError('Payment data is required for Razorpay payments', 400));
    }
  ...
  // Add payment ID if Razorpay
  if (paymentMethod === 'razorpay') {
    bookingData.paymentId = paymentData.razorpay_payment_id;
  }
...
// @route   POST /api/v1/bookings/create-razorpay-order
// @access  Private
